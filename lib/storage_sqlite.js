...
var db, stmt_series_get, stmt_series_ins, stmt_series_upd, stmt_reading_ins;
...
exports.record = function (source, source_key, item, reading) {
  db.serialize(function(){
    ...
    db.run("INSERT INTO series_reading(series_id, recorded_date, reading) SELECT id as series_id,date('now'),? FROM series WHERE source=? and source_key=?", reading, source, source_key);
  });
};
